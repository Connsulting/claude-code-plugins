...
def _message_details(node: ast.AST | None) -> tuple[str, bool, bool]:
    if node is None:
        return "", False, False
    ...
    if isinstance(node, ast.JoinedStr):
        ...
        for value in node.values:
            if isinstance(value, ast.Constant) and isinstance(value.value, str):
                ...
                dynamic = True
        return "".join(parts), dynamic, dynamic and not "".join(parts).strip()
...
def _diagnostic_is_low_context(diagnostic: DiagnosticCall) -> bool:
    ...
    if "$" in diagnostic.message_text or "{" in diagnostic.message_text:
        return False

    normalized = _normalize_message(diagnostic.message_text)
    return bool(normalized) and normalized in GENERIC_DIAGNOSTIC_MESSAGES
...
def _diagnostic_from_call(node: ast.Call) -> DiagnosticCall | None:
    ...
    if name == "print":
        if not _print_targets_stderr(node):
            return None
        message_text, dynamic, exception_only = _message_details(node.args[0] if node.args else None)
...
def _human_stdout_call(node: ast.Call) -> bool:
    ...
    text, _, _ = _message_details(node.args[0])
...
def _analyze_python_try_handlers(source: SourceFile, text: str, tree: ast.AST) -> list[dict[str, Any]]:
    ...
    for node in ast.walk(tree):
        ...
        for handler in node.handlers:
            ...
            if not diagnostics and not _handler_has_structured_error_output(handler) and not _handler_reraises(handler):
