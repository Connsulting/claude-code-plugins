...
def find_duplicate_clusters(
    conn, config: Dict[str, Any], limit: int = 20
) -> List[Dict[str, Any]]:
    ...
    for doc_id in all_ids:
        ...
        if len(cluster_ids) >= 2:
            ...
            clusters.append({"files": files, "count": len(files)})
...
def _search_all_scopes(conn, query_text: str, n_results: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        return [
            {"id": row["id"], "distance": float(row["distance"]) / 2.0} for row in rows
        ]
    except Exception:
        return []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode",
        choices=["all", "duplicates", "outdated"],
        default="all",
        help="Discovery mode",
    )
    parser.add_argument(
        "--limit", type=int, default=20, help="Max items per category (default: 20)"
    )
