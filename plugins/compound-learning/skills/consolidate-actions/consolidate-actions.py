            ...
            if file_path and os.path.exists(file_path):
                ...
                if backup_path:
                    results["backed_up"].append(
                        {"original": file_path, "backup": backup_path}
                    )
                os.remove(file_path)
        ...
        for i, doc_id in enumerate(docs["ids"]):
            ...
            if file_path and os.path.exists(file_path):
                ...
                results["archived"].append(
                    {"original": file_path, "archived_to": archive_path}
                )
            else:
                db.delete_document(conn, doc_id)
                results["archived"].append(
                    {"id": doc_id, "note": "Removed from DB only (file not found)"}
                )
...
def action_rescope(
    doc_id: str, new_scope: str, config: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        ...
        if current_scope == new_scope:
            ...
            return {
                "status": "error",
                "message": f"Document already has scope: {new_scope}",
            }

        filename = (
            os.path.basename(old_file_path)
            if old_file_path
            else f"learning-{doc_id[:8]}.md"
        )
        ...
        if new_scope == "global":
            ...
            return {
                "status": "error",
                "message": "Rescoping to repo requires specifying target repo path",
            }
...
def action_merge(
    ...
    try:
        ...
        merged_tags = (
            ", ".join(sorted(all_tags)[:8]) if all_tags else name.replace("-", ", ")
        )
...
def main():
    ...
    delete_parser.add_argument(
        "--ids", required=True, help="Comma-separated document IDs"
    )
    ...
    rescope_parser.add_argument(
        "--scope", required=True, choices=["global"], help="New scope"
    )
    ...
    merge_parser.add_argument(
        "--name", required=True, help="Name for merged file (kebab-case)"
    )
    merge_parser.add_argument(
        "--output-dir", help="Override output directory (bypasses scope logic)"
    )
    merge_parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.action == "get":
        ...
        result = action_merge(
            ids, args.name, config, output_dir=output_dir, dry_run=dry_run
        )
    else:
