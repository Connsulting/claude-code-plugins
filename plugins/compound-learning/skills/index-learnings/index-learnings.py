...
git_utils = importlib.import_module("lib.git_utils")
...
def generate_manifest(
    manifest_data: Dict[str, List[Dict[str, Any]]], config: Dict[str, Any]
) -> None:
...
def _format_section(title: str, learnings: List[Dict[str, Any]]) -> List[str]:
    ...
    topics: Dict[str, Dict] = defaultdict(
        lambda: {"count": 0, "keywords": defaultdict(int), "gotchas": 0}
    )
    ...
    for topic, data in sorted(
        topics.items(), key=lambda x: x[1]["count"], reverse=True
    ):
        count = data["count"]
...
def index_learning_files():
    ...
    print(
        f"Found {len(all_files)} files (Global: {len(global_files)}, Repos: {len(repo_files)})"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file", metavar="PATH", help="Index a single file instead of all learnings"
    )
