...
The local API uses plain HTTP on port `8765`. Derive the MagicDNS name at handoff time. If `tailscale serve status --json` shows HTTPS on `443` proxying `/` to `http://127.0.0.1:8765`, give reviewers `https://<MagicDNS-name>/<encoded-path>` with no explicit port, after a normal HTTPS GET to that exact page returns HTTP 200. The installer does not configure `tailscale serve`. Without that mapping, use the direct `http://<MagicDNS-name>:8765/<encoded-path>` URL, or `http://127.0.0.1:8765/<encoded-path>` when Tailscale is unavailable.
