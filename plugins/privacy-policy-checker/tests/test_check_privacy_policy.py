...
def test_no_analytics_trackers_detects_escaped_tracker_domains(tmp_path):
    repo_root = tmp_path / "repo"
    config_path = repo_root / "plugins" / "privacy-policy-checker" / ".claude-plugin" / "config.json"
    ...
    _write(repo_root / "PRIVACY_POLICY.md", "[privacy-claim:no_analytics_trackers]\n")
    _write(
        repo_root / "src" / "tracker_regex.js",
        "const trackerDomainRegex = /google-analytics\\.com|cdn\\.segment\\.com/i;\n",
    )
